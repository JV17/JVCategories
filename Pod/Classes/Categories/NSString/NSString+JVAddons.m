...
@implementation NSString (JVAddons)
...
+ (NSString *)reverseString:(NSString *)str
{
    ...
    NSMutableString *mutableString = [NSMutableString stringWithCapacity:len];
    
    while (len > (NSUInteger)0)
    {
        unichar uch = [str characterAtIndex:--len];
        [mutableString appendString:[NSString stringWithCharacters:&uch length:1]];
    }
...
+ (NSString *)commonSuffixWithString:(NSString *)aString options:(NSStringCompareOptions)mask
{
    ...
    NSString *reverse = [self reverseString:aString];
    NSString *commonPrefix = [reverse commonPrefixWithString:[self reverseString:aString] options:mask];
    
    return [self reverseString:commonPrefix];
}
...
- (NSString *)timeLengthToString:(NSInteger)timeLength
{
    ...
    if (timeLength >= 60 * 60 * 24)
    {
        if (timeLength == 60 * 60 * 24)
        {
            timeStr = NSLocalizedStringWithDefaultValue(@"Time Day", @"Generic", [NSBundle mainBundle], @"1 day", @"The format for describing a number of days, if the number is 1, e.g '1 day'");
        }
        else
        {
    ...
    if (timeLength >= 60 * 60)
    {
        ...
        if (m == 1)
        {
            prefix = NSLocalizedStringWithDefaultValue(@"Time Hour", @"Generic", [NSBundle mainBundle], @"1 hour", @"The format for describing a number of hours, if the number is 1, e.g '1 hour'");
        }
        else
        {
    ...
    else
    {
        ...
        if (m != 0)
        {
            ...
            if (m == 1)
            {
                prefix = NSLocalizedStringWithDefaultValue(@"Time Minute", @"Generic", [NSBundle mainBundle], @"1 minute", @"The format for describing a number of minutes, if the number is 1, e.g '1 minute'");
            }
            else
            {
...
- (NSString *)timeLengthToShortString:(NSInteger)timeLength
{
    ...
    if (timeLength >= 60 * 60 * 24)
    {
        if (timeLength == 60 * 60 * 24)
        {
            prefix = NSLocalizedStringWithDefaultValue(@"Time Day", @"Generic", [NSBundle mainBundle], @"1 day", @"The format for describing a number of days, if the number is 1, e.g '1 day'");
        }
        else
        {
    ...
    else
    {
        ...
        prefix = [NSString stringWithFormat:([prefix isEqualToString:@""] ? @"%@%ld:%02ld" : @"%@, %ld:%02ld"), prefix, (long)hours, (long)minutes];
    }
...
- (NSString *)timeIntervalToString:(NSInteger)timeInterval
{
    if (timeInterval == 0)
    {
        return NSLocalizedStringWithDefaultValue(@"Time None", @"Generic", [NSBundle mainBundle], @"None", @"Text that describes a time interval of zero length");
    }
    ...
    NSString *timeLen = [self timeLengthToString:timeInterval];
